...
const lt = "0.4.1", Ae = (t) => typeof t == "string" && /^data:image\/(jpeg|png);base64,[A-Za-z0-9+/=]+$/.test(t), be = {
  new: "Новый",
  active: "Активный",
...
const ct = (t) => JSON.stringify([
  ...
  t.equalProperty,
  t.includeHidden
]), pt = (t, e) => JSON.stringify([t, e].sort());
function ut(t, e, i) {
...
function De(t, e) {
  ...
  return {
    ...i,
    left: De(t, e.slice(0, a)),
    right: De(t, e.slice(a))
  };
}
function* ke(t, e, i) {
...
const Ze = (t, e) => t.bounds.min.some((i, s) => t.bounds.max[s] - i <= e);
function bt(t, e, i) {
...
function $e(t, e, i, s) {
  ...
  for (const f of ke(i, { min: t, max: t }, s))
    if (bt(t, Se(e, f), s)) return !1;
  const r = [1, 0.371390676, 0.52999894], n = ae(W(e.bounds.max, e.bounds.min)) * 3 + 1, a = Be(t, r, n), p = [], h = Pe([...t, ...a]);
...
async function yt(t, e, i, s, r) {
  ...
  const u = async () => {
    ...
    for (let R = 0; R < Me(k); R++)
      Z.push(
        [0, 3, 6].map(
          (me) => [0, 1, 2].map((P) => Math.round(ue(k, R, me + P) / n)).join(",")
        ).sort().join(";")
      ), R % 9e3 === 0 && await u();
    return A = Z.sort().join("|"), M.set(k.id, A), A;
  ...
  async function g(k, A) {
    ...
    for (const [R, me] of z)
      R !== A && T > 96 * 1024 * 1024 && (z.delete(R), T -= U(me), f.delete(R), M.delete(R));
    return Z = await r(k.id), z.set(k.id, Z), T += U(Z), Z;
  }
  let se = -1 / 0;
  for (let k = 0; k < p.length; k++) {
    ...
    const Z = [...ke(ie, A.bounds, n)];
    for (let R = 0; R < Z.length; R++) {
      const me = Z[R];
      performance.now() - se > 150 && (se = performance.now(), i({
        phase: `Проверка пар · A ${k + 1}/${p.length} · кандидаты ${R + 1}/${Z.length}`,
        done: k,
        total: p.length,
        found: E.length
      }));
      const P = h[me];
      if (await u(), A.id === P.id || !Ce(A.bounds, P.bounds, n) || e.ignoreSameModel && A.modelId === P.modelId || e.ignoreSameGroup && A.modelId === P.modelId && A.properties.Объект && A.properties.Объект === P.properties.Объект || e.equalProperty && A.properties[e.equalProperty] !== void 0 && A.properties[e.equalProperty] === P.properties[e.equalProperty] || A.id > P.id && $.has(P.id) && O.has(A.id)) continue;
      const ye = pt(A.id, P.id), j = await g(A), N = await g(P, A.id);
      let F, J = "surface", K = 0;
      if (e.type === "duplicates") {
        if (Me(j) !== Me(N) || j.bounds.min.some(
          (H, q) => Math.abs(H - N.bounds.min[q]) > n || Math.abs(j.bounds.max[q] - N.bounds.max[q]) > n
        ))
          continue;
        await I(j) === await I(N) && (F = j.bounds.min.map((H, q) => (H + j.bounds.max[q]) / 2), J = "duplicate");
      } else {
        const H = w(j), q = w(N), _ = {
          min: j.bounds.min.map(
            (B, Q) => Math.max(B, N.bounds.min[Q])
          ),
          max: j.bounds.max.map(
            (B, Q) => Math.min(B, N.bounds.max[Q])
          )
        }, ce = _.min.map(
          (B, Q) => (B + _.max[Q]) / 2
        ), le = new gt();
        let pe = 1 / 0, he = 0;
        for (const [B, Q] of xe(H, q, n)) {
          const oe = Se(j, B), G = Se(N, Q);
          if (!Ce(Pe(oe.flat()), Pe(G.flat()), n)) continue;
          const V = ht(oe, G, n, e.touching);
          if (V) {
            const Y = ae(W(V, ce));
            (!F || Y < pe) && (F = V, pe = Y), le.add(oe), le.add(G);
          }
        ...
        if (!F && j.closed && N.closed) {
          const B = j.bounds.min.map(
            (Q, oe) => (Q + j.bounds.max[oe]) / 2
          );
          $e(B, j, H, n) && $e(B, N, q, n) && (F = B, J = "contained");
        }
        if (!F) {
          for (const [B, Q, oe] of [
            [j, N, q],
            ...
            if (Q.closed) {
              for (let G = 0; G < Me(B) && !F; G++) {
                const V = Se(B, G), Y = V[0].map(
                  (ge, fe) => (V[0][fe] + V[1][fe] + V[2][fe]) / 3
                );
                for (const ge of [V[0], Y])
                  if ($e(ge, Q, oe, n)) {
                    F = ge, J = "contained";
                    break;
                  }
        ...
        if (F) {
          const B = [...ke(H, _, n)], Q = [...ke(q, _, n)];
          J !== "surface" && (le.addFrom(j, B), le.addFrom(N, Q)), await u();
          const oe = xt(j, N, B, Q, le.values(), _) * 1e3, G = !Ze(j, n) && !Ze(N, n);
          if (oe <= 0 && !e.touching && G) continue;
...
const et = '(function(){"use strict";const cn=({triangles:t,vertices:n,indices:i,triangleCount:a,closed:f,bounds:e,...m})=>m;function dn(t,n){return n.exclude.includes(t.id)?!1:n.include.includes(t.id)?!0:!(n.manualOnly||n.modelsMode==="selected"&&!n.models.includes(t.modelId)||n.modelsMode===void 0&&n.models.length&&!n.models.includes(t.modelId))}const yn=(t,n)=>JSON.stringify([t,n].sort()),M=(t,n)=>[t[0]-n[0],t[1]-n[1],t[2]-n[2]],Y=(t,n,i=1)=>[t[0]+n[0]*i,t[1]+n[1]*i,t[2]+n[2]*i],E=(t,n)=>t[0]*n[0]+t[1]*n[1]+t[2]*n[2],J=(t,n)=>[t[1]*n[2]-t[2]*n[1],t[2]*n[0]-t[0]*n[2],t[0]*n[1]-t[1]*n[0]],I=t=>Math.hypot(...t),gn=t=>{const n=I(t);return n>1e-20?[t[0]/n,t[1]/n,t[2]/n]:void 0},K=t=>t.triangleCount??(t.indices?t.indices.length/3:t.triangles.length/9),T=(t,n,i)=>t.indices&&t.vertices?t.vertices[t.indices[n*3+Math.floor(i/3)]*3+i%3]:t.triangles[n*9+i],B=(t,n)=>[0,3,6].map(i=>[T(t,n,i),T(t,n,i+1),T(t,n,i+2)]);function Z(t){const n=[1/0,1/0,1/0],i=[-1/0,-1/0,-1/0];for(let a=0;a<t.length;a++){const f=a%3;n[f]=Math.min(n[f],t[a]),i[f]=Math.max(i[f],t[a])}return{min:n,max:i}}const W=(t,n,i)=>t.min.every((a,f)=>a<=n.max[f]+i&&t.max[f]>=n.min[f]-i);function k(t,n){const i={min:[1/0,1/0,1/0],max:[-1/0,-1/0,-1/0]};for(const r of n)for(let s=0;s<9;s++){const c=s%3,o=T(t,r,s);i.min[c]=Math.min(i.min[c],o),i.max[c]=Math.max(i.max[c],o)}if(n.length<=12)return{...i,ids:n};const a=i.max.map((r,s)=>r-i.min[s]),f=a.indexOf(Math.max(...a)),e=r=>T(t,r,f)+T(t,r,f+3)+T(t,r,f+6);n.sort((r,s)=>e(r)-e(s));const m=n.length>>1;return{...i,left:k(t,n.slice(0,m)),right:k(t,n.slice(m))}}function*H(t,n,i){W(t,n,i)&&(t.ids?yield*t.ids:(yield*H(t.left,n,i),yield*H(t.right,n,i)))}function*C(t,n,i){if(W(t,n,i)){if(t.ids&&n.ids){for(const a of t.ids)for(const f of n.ids)yield[a,f];return}if(t.ids){yield*C(t,n.left,i),yield*C(t,n.right,i);return}if(n.ids){yield*C(t.left,n,i),yield*C(t.right,n,i);return}yield*C(t.left,n.left,i),yield*C(t.left,n.right,i),yield*C(t.right,n.left,i),yield*C(t.right,n.right,i)}}function nn(t,n){const i={min:[1/0,1/0,1/0],max:[-1/0,-1/0,-1/0]};for(const m of n)for(let r=0;r<3;r++)i.min[r]=Math.min(i.min[r],t[m].bounds.min[r]),i.max[r]=Math.max(i.max[r],t[m].bounds.max[r]);if(n.length<=16)return{...i,ids:n};const a=i.max.map((m,r)=>m-i.min[r]),f=a.indexOf(Math.max(...a));n.sort((m,r)=>t[m].bounds.min[f]+t[m].bounds.max[f]-(t[r].bounds.min[f]+t[r].bounds.max[f]));const e=n.length>>1;return{...i,left:nn(t,n.slice(0,e)),right:nn(t,n.slice(e))}}function tn(t,n,i,a){const f=M(n,t),e=M(i[1],i[0]),m=M(i[2],i[0]),r=J(f,m),s=E(e,r);if(Math.abs(s)<=1e-12*I(f)*I(e)*I(m))return;const c=1/s,o=M(t,i[0]),d=E(o,r)*c,u=J(o,e),y=E(f,u)*c,x=E(m,u)*c,g=a/Math.max(I(e),I(m),a);if(d>=-g&&y>=-g&&d+y<=1+g&&x>=-g&&x<=1+g)return Y(t,f,Math.max(0,Math.min(1,x)))}function xn(t,n,i,a){const f=i.map(Math.abs).indexOf(Math.max(...i.map(Math.abs))),e=[0,1,2].filter(s=>s!==f),m=(s,c,o)=>(c[e[0]]-s[e[0]])*(o[e[1]]-s[e[1]])-(c[e[1]]-s[e[1]])*(o[e[0]]-s[e[0]]),r=(s,c)=>{const o=c.map((d,u)=>m(d,c[(u+1)%3],s));return o.every(d=>d>=-a*I(i))||o.every(d=>d<=a*I(i))};for(const s of t)if(r(s,n))return s;for(const s of n)if(r(s,t))return s;for(let s=0;s<3;s++)for(let c=0;c<3;c++){const o=t[s],d=t[(s+1)%3],u=n[c],y=n[(c+1)%3],x=M(d,o),g=M(y,u),j=x[e[0]]*g[e[1]]-x[e[1]]*g[e[0]];if(Math.abs(j)<1e-18)continue;const F=M(u,o),L=(F[e[0]]*g[e[1]]-F[e[1]]*g[e[0]])/j,P=(F[e[0]]*x[e[1]]-F[e[1]]*x[e[0]])/j;if(L>=0&&L<=1&&P>=0&&P<=1)return Y(o,x,L)}}function Mn(t,n,i,a){const f=J(M(t[1],t[0]),M(t[2],t[0])),e=J(M(n[1],n[0]),M(n[2],n[0])),m=I(f),r=I(e);if(m<1e-20||r<1e-20)return;const s=n.map(o=>E(M(o,t[0]),f)/m),c=t.map(o=>E(M(o,n[0]),e)/r);if(!(s.every(o=>o>i)||s.every(o=>o<-i)||c.every(o=>o>i)||c.every(o=>o<-i))){if(s.every(o=>Math.abs(o)<=i)&&c.every(o=>Math.abs(o)<=i))return a?xn(t,n,f,i):void 0;if(!(!a&&(!(Math.min(...s)<-i&&Math.max(...s)>i)||!(Math.min(...c)<-i&&Math.max(...c)>i))))for(let o=0;o<3;o++){const d=tn(t[o],t[(o+1)%3],n,i);if(d)return d;const u=tn(n[o],n[(o+1)%3],t,i);if(u)return u}}}class pn{items=new Map([["10000,0,0",[1,0,0]],["0,10000,0",[0,1,0]],["0,0,10000",[0,0,1]]]);add(n){if(this.items.size>=256)return;const i=gn(J(M(n[1],n[0]),M(n[2],n[0])));if(!i)return;const f=i[0]<-1e-9||Math.abs(i[0])<=1e-9&&(i[1]<-1e-9||Math.abs(i[1])<=1e-9&&i[2]<0)?[-i[0],-i[1],-i[2]]:[i[0],i[1],i[2]];this.items.set(f.map(e=>Math.round(e*1e4)).join(","),f)}addFrom(n,i){for(const a of i)this.add(B(n,a))}values(){return[...this.items.values()]}}function wn(t,n,i,a,f,e){const m=c=>{let o=1/0,d=-1/0;for(let u=0;u<8;u++){const y=(u&1?e.max[0]:e.min[0])*c[0]+(u&2?e.max[1]:e.min[1])*c[1]+(u&4?e.max[2]:e.min[2])*c[2];y<o&&(o=y),y>d&&(d=y)}return[o,d]},r=(c,o,d,u,y)=>{let x=1/0,g=-1/0;for(const j of o)for(let F=0;F<9;F+=3){const L=T(c,j,F)*d[0]+T(c,j,F+1)*d[1]+T(c,j,F+2)*d[2],P=L<u?u:L>y?y:L;P<x&&(x=P),P>g&&(g=P)}return x===1/0?[u,y]:[x,g]};let s=1/0;for(const c of f){const[o,d]=m(c),u=r(t,i,c,o,d),y=r(n,a,c,o,d),x=Math.min(u[1],y[1])-Math.max(u[0],y[0]);if(x<=0)return 0;x<s&&(s=x)}return Number.isFinite(s)?s:0}const ln=(t,n)=>t.bounds.min.some((i,a)=>t.bounds.max[a]-i<=n);function In(t,n,i){const a=M(n[1],n[0]),f=M(n[2],n[0]),e=J(a,f),m=I(e);if(m<1e-20||Math.abs(E(M(t,n[0]),e))/m>i)return!1;const r=M(t,n[0]),s=E(a,a),c=E(a,f),o=E(f,f),d=E(r,a),u=E(r,f),y=s*o-c*c;if(Math.abs(y)<1e-30)return!1;const x=(d*o-u*c)/y,g=(u*s-d*c)/y,j=i/Math.max(I(a),I(f),i);return x>=-j&&g>=-j&&x+g<=1+j}function en(t,n,i,a){if(!n.closed||t.some((d,u)=>d<=n.bounds.min[u]+a||d>=n.bounds.max[u]-a))return!1;for(const d of H(i,{min:t,max:t},a))if(In(t,B(n,d),a))return!1;const f=[1,.371390676,.52999894],e=I(M(n.bounds.max,n.bounds.min))*3+1,m=Y(t,f,e),r=[],s=Z([...t,...m]);for(const d of H(i,s,a)){const u=tn(t,m,B(n,d),a);if(u){const y=I(M(u,t));y>a&&r.push(y)}}r.sort((d,u)=>d-u);let c=0,o=-1/0;for(const d of r)d-o>a*2&&(c++,o=d);return c%2===1}async function vn(t,n,i,a,f){const e=n.precision/1e3;if(!Number.isFinite(e)||e<=0)throw Error("Точность расчёта должна быть положительным числом.");const m=t.filter(l=>n.includeHidden||!l.hidden),r=m.filter(l=>dn(l,n.a)),s=m.filter(l=>dn(l,n.b));if(!r.length||!s.length)throw Error("Выборка А или Б не содержит элементов. Отметьте хотя бы одну модель в каждой стороне проверки.");let c=performance.now();const o=async()=>{if(a())throw Error("Расчёт отменён. Предыдущие результаты сохранены.");performance.now()-c>16&&(await new Promise(l=>setTimeout(l,0)),c=performance.now())},d=new Map,u=l=>{let h=d.get(l.id);return h||(h=k(l,Array.from({length:K(l)},(q,v)=>v)),d.set(l.id,h)),h},y=new Map,x=async l=>{let h=y.get(l.id);if(h!==void 0)return h;const q=[];for(let v=0;v<K(l);v++)q.push([0,3,6].map(Q=>[0,1,2].map(O=>Math.round(T(l,v,Q+O)/e)).join(",")).sort().join(";")),v%9e3===0&&await o();return h=q.sort().join("|"),y.set(l.id,h),h},g=[],j=new Set(r.map(l=>l.id)),F=new Set(s.map(l=>l.id)),L=nn(s,s.map((l,h)=>h)),P=new Map;let rn=0;const un=l=>l.triangles.byteLength+(l.vertices?.byteLength||0)+(l.indices?.byteLength||0)+K(l)*32;async function mn(l,h){if(!f)return l;let q=P.get(l.id);if(q)return P.delete(l.id),P.set(l.id,q),q;for(const[v,Q]of P)v!==h&&rn>96*1024*1024&&(P.delete(v),rn-=un(Q),d.delete(v),y.delete(v));return q=await f(l.id),P.set(l.id,q),rn+=un(q),q}let D=-1/0;for(let l=0;l<r.length;l++){const h=r[l];performance.now()-D>150&&(D=performance.now(),i({phase:"Проверка пар",done:l,total:r.length,found:g.length}));const q=[...H(L,h.bounds,e)];for(let v=0;v<q.length;v++){const Q=q[v];performance.now()-D>150&&(D=performance.now(),i({phase:`Проверка пар · A ${l+1}/${r.length} · кандидаты ${v+1}/${q.length}`,done:l,total:r.length,found:g.length}));const O=s[Q];if(await o(),h.id===O.id||!W(h.bounds,O.bounds,e)||n.ignoreSameModel&&h.modelId===O.modelId||n.ignoreSameGroup&&h.modelId===O.modelId&&h.properties.Объект&&h.properties.Объект===O.properties.Объект||n.equalProperty&&h.properties[n.equalProperty]!==void 0&&h.properties[n.equalProperty]===O.properties[n.equalProperty]||h.id>O.id&&j.has(O.id)&&F.has(h.id))continue;const qn=yn(h.id,O.id),p=await mn(h),b=await mn(O,h.id);let S,R="surface",sn=0;if(n.type==="duplicates"){if(K(p)!==K(b)||p.bounds.min.some((G,A)=>Math.abs(G-b.bounds.min[A])>e||Math.abs(p.bounds.max[A]-b.bounds.max[A])>e))continue;await x(p)===await x(b)&&(S=p.bounds.min.map((G,A)=>(G+p.bounds.max[A])/2),R="duplicate")}else{const G=u(p),A=u(b),U={min:p.bounds.min.map((w,_)=>Math.max(w,b.bounds.min[_])),max:p.bounds.max.map((w,_)=>Math.min(w,b.bounds.max[_]))},_n=U.min.map((w,_)=>(w+U.max[_])/2),V=new pn;let hn=1/0,Pn=0;for(const[w,_]of C(G,A,e)){const $=B(p,w),N=B(b,_);if(!W(Z($.flat()),Z(N.flat()),e))continue;const z=Mn($,N,e,n.touching);if(z){const X=I(M(z,_n));(!S||X<hn)&&(S=z,hn=X),V.add($),V.add(N)}++Pn%256===0&&(performance.now()-D>150&&(D=performance.now(),i({phase:`Геометрия пары · A ${l+1}/${r.length}`,done:l,total:r.length,found:g.length})),await o())}if(!S&&p.closed&&b.closed){const w=p.bounds.min.map((_,$)=>(_+p.bounds.max[$])/2);en(w,p,G,e)&&en(w,b,A,e)&&(S=w,R="contained")}if(!S){for(const[w,_,$]of[[p,b,A],[b,p,G]])if(_.closed){for(let N=0;N<K(w)&&!S;N++){const z=B(w,N),X=z[0].map((an,fn)=>(z[0][fn]+z[1][fn]+z[2][fn])/3);for(const an of[z[0],X])if(en(an,_,$,e)){S=an,R="contained";break}await o()}if(S)break}}if(S){const w=[...H(G,U,e)],_=[...H(A,U,e)];R!=="surface"&&(V.addFrom(p,w),V.addFrom(b,_)),await o();const $=wn(p,b,w,_,V.values(),U)*1e3,N=!ln(p,e)&&!ln(b,e);if($<=0&&!n.touching&&N)continue;sn=n.touching?$:Math.max(n.precision,$),await o()}if(S&&sn+n.precision<n.minPenetration)continue}if(S&&(g.push({id:qn,a:cn(p),b:cn(b),point:S,kind:R,state:"new",note:"",assignee:"",firstSeen:"",lastSeen:"",penetrationMm:sn}),g.length>=5e4))throw Error("Найдено 50 000 конфликтов. Разделите проверку на меньшие наборы. Незавершённый расчёт не заменил прошлый результат.")}}return i({phase:"Готово",done:r.length,total:r.length,found:g.length}),g}let bn=0;const on=new Map;self.onmessage=async t=>{if(t.data.request!==void 0){const n=on.get(t.data.request);on.delete(t.data.request),t.data.error?n?.reject(Error(t.data.error)):n?.resolve(t.data.geometry);return}try{const{elements:n,check:i}=t.data,a=await vn(n,i,f=>self.postMessage({progress:f}),()=>!1,t.data.streaming?f=>new Promise((e,m)=>{const r=bn++;on.set(r,{resolve:e,reject:m}),self.postMessage({load:f,request:r})}):void 0);self.postMessage({results:a})}catch(n){self.postMessage({error:n instanceof Error?n.message:String(n)})}}})();\n', Qe = typeof self < "u" && self.Blob && new Blob(["(self.URL || self.webkitURL).revokeObjectURL(self.location.href);", et], { type: "text/javascript;charset=utf-8" });
function wt(t) {
  let e;
  try {
    if (e = Qe && (self.URL || self.webkitURL).createObjectURL(Qe), !e) throw "";
...
const Mt = "data:image/png;base64,", St = ":host{display:block;height:100%;color-scheme:dark;font:13px/1.4 Segoe UI,sans-serif;color:#dbe5f0;container-type:size}*{box-sizing:border-box}[hidden]{display:none!important}main{height:100%;min-height:260px;background:#171d25;display:flex;flex-direction:column;overflow:hidden;-webkit-user-select:none;user-select:none}button,input,select,textarea{font:inherit;color:inherit;border:1px solid #425165;border-radius:6px;background:#253140;padding:7px 9px;min-width:0}button{cursor:pointer;white-space:nowrap}button:hover{background:#354459}button:disabled{opacity:.45;cursor:default}.primary{background:#b99a38;border-color:#d0ae46;color:#111820;font-weight:600}.primary:hover{background:#e0bd51}button:focus-visible,input:focus-visible,select:focus-visible,textarea:focus-visible{outline:2px solid #f2c94c;outline-offset:1px}input,select,textarea{background:#111a24}input,textarea,td,dd{-webkit-user-select:text;user-select:text}input[type=checkbox]{accent-color:#e6c354;width:16px;height:16px}header{display:flex;gap:7px;align-items:center;padding:8px 12px;border-bottom:1px solid #354459;flex-shrink:0;overflow-x:auto}.brand{display:flex;gap:7px;align-items:center;margin-right:8px}.brand img{width:32px;height:32px;object-fit:contain}.brand b{font-size:17px}.brand small{font-size:10px;color:#a6b5c7}#dirty{margin-left:auto;font-size:11px;color:#cabb81;white-space:nowrap}.notice{background:#1c2735;padding:5px 12px;font-size:12px;min-height:27px;max-height:70px;overflow:auto;color:#a7bbd1}.notice.error{color:#ffaaa1;background:#412725}.workspace{display:grid;grid-template-columns:230px minmax(0,1fr);flex:1;min-height:0}aside{padding:10px;display:flex;flex-direction:column;gap:9px;border-right:1px solid #354459;min-height:0}#checks{overflow:auto;flex:1;min-height:0}.check-item{display:flex;flex-direction:column;gap:5px;width:100%;margin-bottom:5px;text-align:left;white-space:normal}.check-item small{color:#a8b7ca}.check-item.active,.tabs .active{border-color:#c7ab56;background:#403b2c;color:#ffe399}.main{display:flex;flex-direction:column;min-width:0;min-height:0}.test-toolbar{display:flex;gap:7px;padding:8px 12px;align-items:center}.test-toolbar input{flex:1;font-weight:600}.tabs{display:flex;gap:7px;border-bottom:1px solid #354459;padding:0 12px 8px}.tabs button{min-width:105px}#content{flex:1;min-height:0;overflow:auto;display:flex;flex-direction:column}.empty{padding:28px;color:#a4b4c9;text-align:center;margin:auto}h2,h3,p{margin:0 0 10px}h2{font-size:20px}h3{font-size:15px}label{display:flex;flex-direction:column;gap:5px;font-size:12px;color:#b8c6d7}.check{flex-direction:row;align-items:center;gap:6px}small{font-size:11px;color:#a8b7c9}.parameters{display:flex;gap:18px;align-items:end;padding:10px 12px}.parameters input{width:140px}.selection-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:0 12px 12px}.selection{border:1px solid #39495e;border-radius:8px;padding:12px;background:#1b2633;min-width:0}.selection h3{display:flex;justify-content:space-between}.selection h3 span{color:#92c4b2;font-size:12px}.selection select[multiple]{width:100%;min-height:70px}.selection-tools{display:flex;gap:5px;flex-wrap:wrap;margin:8px 0}.selection-tools button{font-size:11px;padding:5px 7px}.selection>label{margin-bottom:8px}.condition{display:grid;grid-template-columns:minmax(80px,1fr) 100px minmax(70px,1fr) 29px;gap:5px;margin:6px 0}.condition button{padding:3px}.rules,.report{padding:18px;max-width:850px}.rules>label{margin:14px 0}.rules>p,.report>p{color:#a9b8cb}.report>button{margin:12px 8px 14px 0}.result-tools{display:flex;gap:7px;flex-wrap:wrap;padding:8px 12px}.result-tools input{flex:1;min-width:180px}.result-area{display:grid;grid-template-columns:minmax(0,1fr) 300px;flex:1;min-height:0}.table-area{display:flex;flex-direction:column;min-width:0;min-height:0}.scroll{overflow:auto;flex:1;min-height:0}table{border-collapse:separate;border-spacing:0;width:max-content;min-width:100%}th{position:sticky;top:0;background:#2a3748;z-index:1;text-align:left;max-width:240px;white-space:normal}th,td{padding:8px;border-bottom:1px solid #344154;vertical-align:top}td{max-width:320px;min-width:90px;overflow-wrap:anywhere;-webkit-user-select:text;user-select:text}td:first-child{min-width:36px}tr.active td{background:#453e29;color:#fff0b5}tr:hover td{background:#2c3847}.pager{display:flex;gap:10px;align-items:center;justify-content:center;padding:5px;border-top:1px solid #39495e;font-size:11px}.pager button{padding:2px 8px}.detail{border-left:1px solid #354459;padding:10px;overflow:auto}.detail h3{overflow-wrap:anywhere}.detail>label{margin:10px 0}.detail p{font:11px/1.5 Consolas,monospace}.detail details{border-top:1px solid #354459;padding:8px 0}summary{cursor:pointer}dl{font-size:11px;display:grid;grid-template-columns:1fr 1fr;gap:7px}dt,dd{overflow-wrap:anywhere;margin:0}dt{color:#9baec4}footer{border-top:1px solid #354459;padding:4px 12px;display:flex;justify-content:space-between;color:#91a4bb;font-size:11px}dialog{border:1px solid #53647c;border-radius:12px;background:#202c3a;color:#dbe5f0;width:min(700px,92vw);max-height:85vh;padding:20px;box-shadow:0 15px 60px #000a}dialog::backdrop{background:#000a}dialog p{margin:12px 0}dialog a{color:#f2c94c;margin-right:12px}.dialog-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:16px}@container (max-width:950px){.workspace{grid-template-columns:185px minmax(0,1fr)}.result-area{grid-template-columns:minmax(0,1fr) 240px}.selection-grid{grid-template-columns:1fr}.parameters,.test-toolbar{flex-wrap:wrap}.test-toolbar input{flex-basis:100%}}@container (max-height:400px){header{padding:5px 10px}.test-toolbar{padding:5px 10px}.tabs{padding-bottom:5px}.brand img{width:25px;height:25px}footer{display:none}}:host{font-size:12px}.brand img{background:#e1e7ef;border-radius:5px;padding:3px}.selection-tools button{font-size:12px}label{font-size:13px}.choose-layout{display:grid;grid-template-columns:220px minmax(0,1fr);min-height:0;flex:1;overflow:auto}.choose-layout .parameters{display:flex;flex-direction:column;align-items:stretch;gap:15px;border-right:1px solid #354459;justify-content:flex-start}.choose-layout .parameters input{width:100%}.choose-layout .parameters input[type=checkbox]{width:16px}.choose-layout .selection-grid{padding:10px 12px;grid-template-columns:minmax(0,1fr) minmax(0,1fr);align-items:start;min-width:0}.selection select[multiple]{height:180px;resize:vertical}.selection-mode{font-size:12px;color:#b6c4d5}.preset-row{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:9px}.preset-row select{flex:1 1 160px}.preset-row button{padding:5px 7px;font-size:12px}.model-list{height:150px;overflow:auto;border:1px solid #425165;border-radius:6px;background:#111a24;padding:5px;margin-bottom:5px}.model-list label{display:flex;flex-direction:row;align-items:center;gap:7px;padding:4px 5px;color:#dbe5f0;overflow-wrap:anywhere}.model-list label:hover{background:#253140}.model-list .model-all{position:sticky;top:-5px;z-index:1;background:#1f2b39;border-bottom:1px solid #425165;font-weight:600}.legend{display:flex;gap:12px;flex-wrap:wrap;font-family:inherit!important}.part-a{color:#ff6161}.part-b{color:#65baff}.preview{display:block;width:100%;padding:5px;margin:8px 0}.preview img{display:block;width:100%;max-height:180px;object-fit:contain}.image-dialog{width:94vw;max-width:1500px}.image-dialog img{display:block;max-width:100%;max-height:75vh;margin:10px auto}#help-dialog{width:min(1000px,94vw);line-height:1.6}#help-dialog details{padding:12px 0;border-bottom:1px solid #425165}#help-dialog summary{font-size:16px;font-weight:600;color:#f0d27a}#help-dialog li{margin:8px 0}@container (max-width:1100px){.choose-layout{grid-template-columns:180px minmax(0,1fr)}.choose-layout .selection-grid{grid-template-columns:1fr}.condition{grid-template-columns:minmax(60px,1fr) 100px minmax(60px,1fr) 29px}}@container (max-width:700px){.choose-layout{display:block}.choose-layout .parameters{border-right:0;border-bottom:1px solid #354459}.workspace{grid-template-columns:160px minmax(0,1fr)}}button,input,select,textarea{padding:5px 7px;border-radius:5px}button{min-height:27px}input[type=checkbox]{width:14px;height:14px}.commandbar{position:relative;display:flex;gap:8px;min-height:45px;padding:5px 8px;overflow:visible;background:#131b25}.brand{flex:0 0 auto;margin-right:0;gap:5px}.brand img{width:28px;height:28px}.brand b{font-size:14px}.check-heading{display:grid;grid-template-rows:25px 12px;flex:0 1 230px;min-width:145px}.check-heading input{height:25px;border:0;background:transparent;padding:2px 5px;font-weight:600}.check-heading small{padding:0 5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:9px}.tabs{align-self:stretch;align-items:center;flex:0 0 auto;gap:3px;padding:0;border:0}.tabs button{flex:0 0 auto;min-width:0;padding:5px 9px;background:transparent;border-color:transparent}.tabs .active{background:#3e3828;border-color:#b99a38}.header-actions{display:flex;align-items:center;gap:4px;margin-left:auto}#dirty{margin-left:0;width:0;height:0;overflow:visible;font-size:0}#dirty:not(:empty){width:7px;height:7px;border-radius:50%;background:#e5bd43;box-shadow:0 0 0 2px #e5bd4328}.more-menu{position:relative;z-index:20}.more-menu summary{display:grid;place-items:center;width:29px;height:29px;border:1px solid #425165;border-radius:5px;background:#253140;cursor:pointer;list-style:none;font-size:18px}.more-menu summary::-webkit-details-marker{display:none}.more-popover{position:absolute;right:0;top:calc(100% + 5px);display:grid;width:190px;gap:3px;padding:6px;border:1px solid #506176;border-radius:7px;background:#192431;box-shadow:0 12px 35px #000a}.more-popover button{text-align:left}.notice{min-height:23px;max-height:45px;padding:3px 9px;font-size:10px}.run-progress{display:grid;grid-template-columns:minmax(150px,auto) minmax(120px,1fr) auto;align-items:center;gap:8px;min-height:24px;padding:3px 9px;border-bottom:1px solid #425165;background:#202a20;color:#d9e7d2;font-size:10px}.run-bar{width:100%;height:7px;overflow:hidden;border-radius:2px;background:#101810}.run-bar i{display:block;width:0;height:100%;background:#e3bc43}#run-phase,#run-value{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}#run-value{text-align:right;color:#b9c9b3}.workspace{grid-template-columns:190px minmax(0,1fr)}aside{gap:6px;padding:7px;background:#141c26}.aside-title{font-weight:600;font-size:12px}.aside-actions{display:grid;gap:5px}.check-item{gap:2px;margin-bottom:3px;padding:6px 7px}.check-item small{font-size:9px}.main{background:#171d25}#content{overflow:hidden}.choose-layout{grid-template-columns:180px minmax(0,1fr)}.choose-layout .parameters{gap:9px;padding:8px}.choose-layout .selection-grid{gap:8px;padding:8px}.selection{padding:8px}.selection h3,h3{font-size:13px;margin-bottom:6px}label{gap:3px;font-size:11px}small,.selection-tools button,.preset-row button{font-size:10px}.model-list{height:125px}.model-list label{padding:2px 4px}.selection-tools{gap:4px;margin:5px 0}.selection-tools button,.preset-row button{padding:3px 5px;min-height:23px}.result-tools{display:flex;align-items:center;justify-content:space-between;gap:6px;flex-wrap:nowrap;min-height:38px;padding:5px 7px;border-bottom:1px solid #354459;background:#151f2a}.result-filters,.bulk-tools{display:flex;align-items:center;gap:5px;min-width:0}.result-filters{flex:1}.result-filters input[type=search]{flex:1 1 190px;min-width:120px}#result-depth{flex:0 1 125px;min-width:92px}#result-count,#selection-count{color:#9fb0c4;white-space:nowrap;font-size:10px}.bulk-tools select{max-width:125px}.result-area{grid-template-columns:minmax(0,1fr) 310px}th,td{padding:5px 6px;font-size:11px;line-height:1.25}th{max-width:190px}td{max-width:260px;min-width:76px}.pager{justify-content:flex-end;min-height:29px;padding:3px 7px}.pager #selection-count{margin-right:auto}.detail{display:flex;flex-direction:column;min-width:0;padding:0;overflow:hidden;background:#141e29}.detail-head{display:flex;align-items:center;justify-content:space-between;gap:6px;min-height:47px;padding:7px 8px 4px}.detail-head>div:first-child{min-width:0}.detail-head small{color:#8093aa;letter-spacing:.08em}.detail-head h3{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin:0}.detail-nav{display:flex;gap:3px}.detail-nav button{min-width:27px;padding:2px 7px;font-size:17px}.clash-summary{display:flex;gap:5px;padding:0 8px 5px;color:#adbdcf;font-size:9px}.clash-summary span{padding-right:5px;border-right:1px solid #405064}.clash-summary span:last-child{border:0}.detail .legend{margin:0;padding:0 8px 5px;font-size:10px}.preview-slot{height:clamp(92px,18cqh,145px);flex:0 0 auto;margin:0 7px 6px;overflow:hidden;border:1px solid #37475b;border-radius:6px;background:#0e151d}.preview{position:relative;width:100%;height:100%;margin:0;padding:0;border:0;background:transparent}.preview img{width:100%;height:100%;max-height:none;object-fit:contain}.preview span{position:absolute;right:5px;bottom:5px;padding:3px 6px;border-radius:4px;background:#101923db;font-size:9px}.preview-empty,.detail-empty{height:100%;display:grid;place-content:center;justify-items:center;gap:4px;color:#788ba1}.preview-empty>span,.detail-empty>span{font-size:24px}.detail-actions{display:grid;grid-template-columns:1fr 1fr;gap:5px;padding:0 7px 7px}.detail-scroll{flex:1;min-height:0;overflow:auto;padding:7px 8px 12px;border-top:1px solid #354459}.detail-scroll>label{margin:7px 0}.detail p{font:inherit}.coordinates{display:grid;grid-template-columns:repeat(3,1fr);gap:3px;font:9px/1.3 Consolas,monospace;color:#91a3b7}.coordinates span{overflow:hidden;text-overflow:ellipsis}.detail details{padding:6px 0}.detail dl{font-size:10px}.detail-empty{padding:20px;text-align:center}footer{padding:2px 8px;font-size:9px}@container (max-width:1050px){.workspace{grid-template-columns:165px minmax(0,1fr)}.result-area{grid-template-columns:minmax(0,1fr) 270px}.check-heading{flex-basis:185px}.tabs button{padding-inline:6px}#dirty{display:none}}@container (max-width:820px){.brand b,.brand small,#result-count{display:none}.check-heading{flex-basis:155px}.result-tools{overflow-x:auto}.result-area{grid-template-columns:minmax(0,1fr) 245px}}@container (max-height:410px){.notice,footer{display:none}.preview-slot{height:90px}}", ve = /* @__PURE__ */ new WeakMap(), tt = "nashepo.collisionfinder360.project.", Oe = () => ({
  ...
  if (!(!t || typeof sessionStorage > "u"))
    try {
      const e = sessionStorage.getItem(tt + t);
...
function It(t, e) {
  const i = t.shadowRoot || t.attachShadow({ mode: "open" }), s = rt(t);
  ...
  i.innerHTML = `<style>${St}</style><main><header class="commandbar"><div class="brand"><img src="${Mt}" alt=""><b>НашеПО</b><small>${lt}</small></div><div class="check-heading"><input id="name" aria-label="Имя проверки" placeholder="Выберите проверку"><small id="check-summary">Проверка не выбрана</small></div><div id="tabs" class="tabs">${[
    ["select", "Выбор"],
    ["results", "Результаты"],
    ["rules", "Правила"],
    ["report", "Отчёт"]
  ...
  const k = (o, l = !1) => {
    ...
    $ = !0, g("dirty").textContent = "Есть несохранённые изменения", r && ve.set(r, a), Je(n, a);
  }, ye = () => {
  ...
  const le = (o) => {
    ...
    const l = new Set(o.map((y) => y.id)), d = o.map((y) => ({
      ...
      try {
        ...
        const C = String(S[0]), D = c(C);
        return D === C ? y : JSON.stringify([D, ...S.slice(1)]);
      } catch {
        return y;
      }
    };
    let v = !1;
    const b = (y) => {
      const S = y.models.map(c), C = y.include.map(x), D = y.exclude.map(x);
      (S.some((ee, te) => ee !== y.models[te]) || C.some((ee, te) => ee !== y.include[te]) || D.some((ee, te) => ee !== y.exclude[te])) && (y.models = [...new Set(S)], y.include = [...new Set(C)], y.exclude = [...new Set(D)], v = !0);
    };
    ...
    for (const y of a.sets) {
      ...
      S.some((C, D) => C !== y.selection.models[D]) && (y.selection.models = [...new Set(S)], v = !0);
    }
    return v && P(), v;
  }, oe = () => {
    const o = U();
  ...
  function V(o, l = !1) {
    if (!w) {
      ...
      if (G(), l) {
        ...
        d && (e.focus(d, Number(g("distance").value)), Y(d));
      }
    }
  }
  function Y(o) {
    clearTimeout(z);
    const l = ++T, d = Number(g("distance").value);
    o.image && o.imageScope === "pair-ab" && o.imageDistance === d || !e.canLocate(o) || (z = window.setTimeout(async () => {
      if (!(l !== T || w || u !== o.id))
        try {
          ...
          if (l !== T || u !== o.id) return;
          o.image = c, o.imageScope = "pair-ab", o.imageDistance = d, P(), m === "results" && ce();
        } catch (c) {
          l === T && u === o.id && k(
  ...
  async function ge(o) {
    ...
    try {
      ...
      o.image = await e.snapshot(o, l, () => M), o.imageScope = "pair-ab", o.imageDistance = l, P(), m === "results" && u === o.id && ce();
    } catch (l) {
      k(
  ...
  async function fe(o, l = !1) {
    ...
    p = await e.scan(
      ...
      () => M,
      d
    ), g("model-count").textContent = `Проиндексировано моделей: ${p.indexedModelIds.length} из ${p.models.length} · элементов: ${p.elements.length}`, q(), k(
    ...
    try {
      ...
      for (const c of l) {
        ...
        c.results = ut(
          ...
          v,
          b
        ), c.lastRun = b, c.fingerprint = p.fingerprint, c.configAtRun = x, c.modelsAtRun = [...p.indexedModelIds], c.status = "done", c.warnings = [...p.warnings], h = c.id, u = c.results[0]?.id || "", O.clear(), P();
      }
      m = "results", q(), G(), k(
  ...
  g("new").onclick = () => {
    const o = dt();
    o.name = `Проверка ${a.checks.length + 1}`, a.checks.push(o), h = o.id, m = "select", u = "", O.clear(), P(), q();
    ...
    l && !w && (e.clear(), h = l.dataset.check, u = "", O.clear(), f = 0, q());
  }, g("tabs").onclick = (o) => {
    const l = o.target.closest("[data-tab]");
    l && !w && (m = l.dataset.tab, q());
  }, g("name").onchange = () => {
    const o = U();
    o && (o.name = g("name").value.trim() || "Проверка", P(), K());
  }, g("copy").onclick = () => {
    const o = U();
    if (!o) return;
    ...
    Object.assign(l, {
      ...
      configAtRun: void 0,
      status: "new"
    }), a.checks.push(l), h = l.id, u = "", O.clear(), P(), q();
  }, g("delete").onclick = () => {
    U() && confirm(`Удалить проверку «${U().name}» и её результаты?`) && (a.checks = a.checks.filter((o) => o.id !== h), h = a.checks[0]?.id || "", O.clear(), e.clear(), P(), q());
    ...
    ) && (a.checks = [], a.sets = [], p = void 0, h = "", u = "", O.clear(), e.clear(), P(), g("model-count").textContent = "Модели не прочитаны", q(), k("Данные проверок текущего проекта очищены."));
  }, g("save").onclick = () => {
    Ve("НашеПО-проверки.json", JSON.stringify(a, null, 2)), $ = !1, g("dirty").textContent = "Файл проверок сохранён";
  }, g("open").onclick = () => g("file").click(), g("file").onchange = () => R(async () => {
    const o = g("file").files?.[0];
    if (!o) return;
    const l = _e(await o.text());
    $ && !confirm("Заменить текущие несохранённые проверки данными из файла?") || (a = l, r && ve.set(r, a), Je(n, a), h = a.checks[0]?.id || "", u = "", O.clear(), e.clear(), $ = !1, g("dirty").textContent = "Проверки открыты", q(), k("Проверки открыты. Обновите модели перед переходом к элементам."), g("file").value = "");
  });
  ...
  g("content").onchange = (o) => R(() => {
    ...
    if ([
      ...
      l.id === "type" && (d.type = l.value), l.id === "touching" && (d.touching = l.checked), l.id === "same-model" && (d.ignoreSameModel = l.checked), l.id === "same-group" && (d.ignoreSameGroup = l.checked), l.id === "hidden" && (d.includeHidden = l.checked), l.id === "equal-property" && (d.equalProperty = l.value), j(), q();
      return;
    }
    ...
    c && (l.id === "edit-state" && (c.state = l.value, _(), K(), G()), l.id === "assignee" && (c.assignee = l.value), l.id === "note" && (c.note = l.value, _()), P());
  }), g("content").oninput = (o) => {
    const l = o.target;
    ...
    if (d?.dataset.selection) {
      ...
      switch (d.dataset.selection) {
        case "load-set": {
          const C = a.sets.find((D) => D.id === b.presetId);
        ...
        case "save-set": {
          ...
          a.sets.push(D), b.presetId = D.id, S = !1;
          break;
        }
        case "delete-set": {
          const C = a.sets.find((D) => D.id === b.presetId);
          ...
          a.sets = a.sets.filter((D) => D.id !== C.id);
          for (const D of a.checks)
            for (const ee of [D.a, D.b])
        ...
        case "include": {
          const C = e.selected();
          if (!C.length) throw Error("Выделите элементы в 3D-сцене.");
          b.include = [.../* @__PURE__ */ new Set([...b.include, ...C])], b.exclude = b.exclude.filter((D) => !C.includes(D));
          break;
        }
        case "exclude": {
          const C = e.selected();
          if (!C.length) throw Error("Выделите элементы в 3D-сцене.");
          b.exclude = [.../* @__PURE__ */ new Set([...b.exclude, ...C])], b.include = b.include.filter((D) => !C.includes(D));
          break;
        }
        case "reset":
          b.manualOnly = !1, b.include = [], b.exclude = [];
      }
      S ? j() : P(), q(), g("content").scrollTop = y;
      return;
    }
    if (d?.id === "prev-page" && (f--, _()), d?.id === "next-page" && (f++, _()), d?.id === "show-markers" && (E = !E, d.textContent = E ? "● Знаки включены" : "○ Знаки выключены", d.setAttribute("aria-checked", String(E)), G()), d?.id === "bulk") {
      ...
      if (v) {
        ...
        try {
          v.image = await e.snapshot(
            ...
            () => M,
            !0
          ), v.imageScope = "pair-ab", v.imageDistance = void 0, P(), ce(), k("Снимок сохранён в результат.");
        } finally {
          Z(), L(!1);
        }
      }
    ...
    if (d?.id === "focus" && V(u, !0), d?.id === "previous" || d?.id === "next") {
    ...
    if (d?.id === "export-html" || d?.id === "export-viewer") {
      ...
      if (g("report-images").checked) {
        const S = e.view, C = S?.storeView(), D = Number(g("distance").value);
        ...
        try {
          await e.captureWorkspace(async () => {
            ...
            for (const te of b) {
              ...
              const ot = te.imageScope !== "pair-ab" || te.imageDistance !== void 0 && te.imageDistance !== D;
              if (!te.image || ot) {
                ...
                try {
                  te.image = await e.snapshot(te, D, () => M), te.imageScope = "pair-ab", te.imageDistance = D, P();
                } catch (at) {
                  if (M || !e.isCurrent()) throw at;
                  v++;
                }
              }
      ...
      Ve(
        ...
        d.id === "export-html" ? vt(c, y) : kt(c, y)
      ), k(
    ...
    d && !l.closest("input") && R(() => V(d.dataset.result, !0));
  };
  const nt = setInterval(() => {
    ...
    ), w || q()) : p && !e.isCurrent() && (p = void 0, e.clear(), g("model-count").textContent = "3D-окно изменилось", k("Активное 3D-окно изменилось. Обновите модели."), w || q()));
  }, 1500);
  return q(), () => {
    s(), clearInterval(nt), clearTimeout(z), T++, M = !0, ie?.(), I?.terminate(), e.clear();
  };
}
...
async function jt(t) {
  await Ue(), t.repaint();
  const { candidates: e, rect: i } = it(t), s = document.createElement("canvas");
...
async function Et(t, e) {
  ...
  const { candidates: i } = it(t), s = document.createElement("canvas"), r = Math.min(1, 1280 / i[0].width);
...
const ze = "nashepo.checks.points", We = "nashepo.checks.highlight";
function He(t) {
...
class Ct {
  ...
  async scan(e, i, s) {
    ...
    const O = (z) => {
      for (let T = 0; T < z.length; T++)
        I = Math.imul(I ^ z.charCodeAt(T), 16777619);
    }, ie = async (z, T, U) => {
      if (M.has(z)) return;
      M.add(z);
      const g = z.layers.layer0?.modelName || T, se = T, k = Ne(g) || Ne(se), A = (j, N) => {
        h.has(j) || (h.add(j), p.push({ id: j, name: N }));
      };
      k || A(se, g);
      const Z = !k && (!s || s.has(se)), R = [];
      (Z || k) && z.layouts.model?.walk((j) => (j.type === Fe.model3d ? R.push(j) : j.type === Fe.insert && m.push(`${g}: вставка блока не включена в расчёт.`), !1));
      const me = /* @__PURE__ */ new Map();
      for (const j of R) {
        let N = j.layer, F = "";
        for (; N; ) {
          if (N.modelName && !Ne(N.modelName)) {
            F = N.modelName;
            break;
          }
          N = N.layer;
        }
        const J = k ? F || "Модель проекта" : g, K = k ? F || `${T}/#model` : se;
      ...
      for (const j of me.values()) {
        const { key: N, objects: F, modelId: J, modelName: K } = j;
        ...
        try {
          if (H) {
            ...
            let V = H;
            for (; V && G.length < 64; )
              G.unshift(V), V = V.layer;
            for (const Y of G)
              Ee(Y.typedProperties(), "", q), Y.typed?.name && (q.Тип = Y.typed.name);
          }
        } catch {
          m.push(`${K} / ${N}: часть свойств недоступна.`);
        }
        const _ = q["ifc.id"] || Object.entries(q).find(
          ([G]) => /(^|\.)(globalid|ifcguid|guid)$/i.test(G)
        )?.[1] || "", ce = H?.name || F[0].$id || "Элемент", le = JSON.stringify([J, N]);
        Object.assign(q, {
          Модель: K,
          Имя: ce,
          GUID: _,
        ...
        for (const G of F) {
          he &&= G.isClosed;
          for (const V of Object.values(G.meshes)) {
            const Y = V.geometry;
            if (!Y || Y.indices.length % 3) {
              B = !0;
              continue;
            }
            he &&= V.isClosed;
            for (let L = 0; L < Y.vertices.length; L += 3) {
              const re = [
                Y.vertices[L],
                Y.vertices[L + 1],
                Y.vertices[L + 2]
              ];
              if (Math3d.mat4.mulv3(re, G.matrix, re), !re.every(Number.isFinite)) {
                B = !0;
                continue;
              }
            ...
            const ge = Y.vertices.length / 3, fe = (L) => Number.isFinite(Y.vertices[L * 3]) && Number.isFinite(Y.vertices[L * 3 + 1]) && Number.isFinite(Y.vertices[L * 3 + 2]);
            for (let L = 0; L < Y.indices.length; L += 3) {
              const re = Y.indices[L], ne = Y.indices[L + 1], we = Y.indices[L + 2];
              if (I = Math.imul(I ^ re, 16777619), I = Math.imul(I ^ ne, 16777619), I = Math.imul(I ^ we, 16777619), re < ge && ne < ge && we < ge && re !== ne && ne !== we && we !== re && fe(re) && fe(ne) && fe(we) ? Q++ : B = !0, L % 15e4 === 0 && (await E(), i()))
                throw Error("Чтение моделей отменено.");
            }
          }
        }
        ...
        O(JSON.stringify([le, q, oe.hidden])), f.push(oe), w.set(le, F);
      }
      P && m.push(
        `${g}: пропущено элементов без треугольной геометрии — ${P}.`
      );
      const ye = [];
      ...
      for (const j of ye) {
        const N = j.name || j.uri || j.$id, F = N || "Подключённая модель", J = `${T}/${N || "attachment"}`;
  ...
  async snapshotInWorkspace(e, i, s, r = !1) {
    ...
    let u;
    try {
      r ? this.highlight(e) : this.focus(e, i, !1), n.pauseAnimation(), u = await jt(n), n.layer.clearSelected(), a.visible = !1, n.annotations.visible = !1, n.invalidate();
      const f = await Et(
        n,
        () => s() || !this.isCurrent()
      );
...
const $t = {
  ...
  mount(t) {
    ...
    i.style.height = "100%", e.replaceChildren(i), qe = i, Ke = t.manager, Xe = It(i, new Ct(t));
  }
};
export {
  $t as default
