      ...
      tests: [
        {
          ...
          clashes: rows.map((r, i) => ({
            ...
            properties: {
              Проверка: check.name,
              Вид: r.kind,
              "Расчётная глубина пересечения, мм": r.unmeasured
                ? "не определена"
                : String(r.penetrationMm ?? 0),
            },
          })),
        },
      ],
